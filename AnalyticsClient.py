...
class AnalyticsClient:
    ...
    def get_access_token(self):
        ...
        """
        url = f"https://accounts.zoho.in/oauth/v2/token?client_id={self.client_id}&client_secret={self.client_secret}&grant_type=refresh_token&refresh_token={self.refresh_token}"
    
        response = requests.post(url)
        access_token = response.json()['access_token']
        return access_token

    def get_access_token_analytics(self):
        """
        Get Access token from zoho
        """
        url = f"https://{self.analytics_server_url}/oauth/v2/token?client_id={self.client_id}&client_secret={self.client_secret}&grant_type=refresh_token&refresh_token={self.refresh_token}"
